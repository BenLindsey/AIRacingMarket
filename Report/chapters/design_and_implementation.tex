...
\begin{tikzpicture}[auto, thick, node distance=2cm, >=triangle 45]
	...
	% Blocks of MIDDLE
	\draw
		node at (5.5,-4) [block, name=suma2] {REST API}
	;

        % Blocks of BACK END
	\draw
		node at (5.5,-8) [db, name=ret2] {\Large MongoDB}
	;
	% Joining BACK END
	...
	%\draw[->] (inte2) -- node {} (suma4);
        ...
        %\draw node at (8,0) {\textbullet};
        
	% Boxing
	\draw [color=gray,thick](-0.5,-3) rectangle (12.5,1);
	\node at (-0.5,1) [above=5mm, right=0mm] {\textsc{Front-End}};
	\draw [color=gray,thick](-0.5,-9) rectangle (12.5,-5);
	\node at (-0.5,-9) [below=5mm, right=0mm] {\textsc{Back-End}};
\end{tikzpicture}
\end{center}
